...
class GapPlot {
    ...
    drawPlot() {
        ...
        this.updatePlot(2000, "gdp", "life-expectancy", "population");
    ...
    updatePlot(activeYear, xIndicator, yIndicator, circleSizeIndicator) {
        ...
        */
        ...
        let Circle_min = this.FindMin(circleSizeIndicator);
        ...
        xIndicator = xIndicator === 'population' ? 'new_population' : xIndicator;
        yIndicator = yIndicator === 'population' ? 'new_population' : yIndicator;
        circleSizeIndicator = circleSizeIndicator === 'population' ? 'new_population' : circleSizeIndicator;
        ...
        {
            // xIndicator
            let xVal = this.getValues(activeYear, xIndicator, i);
            let yVal = this.getValues(activeYear, yIndicator, i);
            let circleSize = this.getValues(activeYear, circleSizeIndicator, i);

            
            let CountryName = xIndicator != 'new_population' ? this.data[xIndicator][i].country : this.data[yIndicator][i].country;
            let CountryId = xIndicator != 'new_population' ? this.data[xIndicator][i].geo : this.data[yIndicator][i].geo;

            let Region = this.data['new_population'][i] ? this.data['new_population'][i].region : null;

            list_of_plot_data.push(new PlotData(CountryName, xVal, yVal, CountryId, Region, circleSize))
        }        
        ...
        circles.attr('cx', d => circle_x_scale(d.xVal))
